...
/* Includes ------------------------------------------------------------------*/
/*此处添加本文件需引用的头文件*/
#include <Protocol_Volant_INT2.h>
#include "stdlib.h"
//#include "MS_Ctrl.h"
...
static CanStdProtocol_FrameInfo_t sg_frameInfoList_INT2[CCSP_FRAME_NUM_INT2] =
{
	// 控制信息
    {.SN = 0, .frameFormat = 1, .frameMode = 2, .dataLength = 12, .direction = 1, .sendType = 0, .interval = 1, \
    .enableFlag = TRUE, .frameId = 0x18EF0E20, .pfDataUpdateFunc = CanProtocol_MCU_INT2_TX_CtrlMsg_Update, .pfTriggerFunc = NULL},

    {.SN = 1, .frameFormat = 1, .frameMode = 2, .dataLength = 12, .direction = 0, .sendType = 0, .interval = 1, \
    .enableFlag = TRUE, .frameId = 0x18EF0E20, .pfDataUpdateFunc = CanProtocol_MCU_INT2_RX_CtrlMsg_Update, .pfTriggerFunc = NULL},

	// 状态信息
    {.SN = 2, .frameFormat = 1, .frameMode = 2, .dataLength = 32, .direction = 1, .sendType = 0, .interval = 1, \
    .enableFlag = TRUE, .frameId = 0x18EF0F02, .pfDataUpdateFunc = CanProtocol_MCU_INT2_TX_StateMsg_Update, .pfTriggerFunc = NULL},

    {.SN = 3, .frameFormat = 1, .frameMode = 2, .dataLength = 32, .direction = 0, .sendType = 0, .interval = 1, \
    .enableFlag = TRUE, .frameId = 0x18EF0F02, .pfDataUpdateFunc = CanProtocol_MCU_INT2_RX_StateMsg_Update, .pfTriggerFunc = NULL}
};

static uint32 sg_eventList_INT2[CCSP_EVENTFRAME_NUM_INT2] =
{
    0,
};
...
CanStdProtocol_t g_VolantInt2Protocol =
{
    .name = CCSP_NAME_INT2,
    .baudrate = CCSP_BAUDRATE_INT2,
    .fastBaudrate = CCSP_FASTBAUDRATE_INT2,
    .deviceType = CCSP_DEVICETYPE_INT2,
    .frameNum = CCSP_FRAME_NUM_INT2,
    .pframeInfoList = sg_frameInfoList_INT2,
    .pEventList = sg_eventList_INT2,
    .pfEventUpdate = CanProtocol_Volant_INT2_Event_Update
};
...
void CanProtocol_MCU_INT2_TX_CtrlMsg_Update(void *pframe)
{
...
//
	...
	/* ----------------------------------CRC------------------------------------- */
	setA825CRC(sg_frameInfoList_INT2[MCU_INT2_TX_CtrlMsg].frameId, sg_frameInfoList_INT2[MCU_INT2_TX_CtrlMsg].dataLength, pframe);
	/* -----------------------------RollingCounter------------------------------- */
	CanVolantData.CanVolantModuleData_2.INT_RollingCounter_C++;
}

void CanProtocol_MCU_INT2_RX_CtrlMsg_Update(void *pframe)
{
	CANFD_INT_CTRL_OBJ *pCanFrame		= (CANFD_INT_CTRL_OBJ *)pframe;

	if(checkA825CRC(sg_frameInfoList_INT2[MCU_INT2_RX_CtrlMsg].frameId, sg_frameInfoList_INT2[MCU_INT2_RX_CtrlMsg].dataLength, pframe))
	{
		CanVolantData.CanVolantModuleData_2.INT_CRC_Ctrl_Fault			= FALSE;

		if(CanVolantData.CanVolantModuleData_2.INT_IsPrimaryModule)
		{
			// 握手信息
...
//			MS_Data.MS_CtrlMsg.CRC_Ctrl									= ((pCanFrame->Data.CRC_Ctrl_H & 0xFF) << 8) + (pCanFrame->Data.CRC_Ctrl_L & 0xFF);
	...
	{
		CanVolantData.CanVolantModuleData_2.INT_CRC_Ctrl_Fault			= TRUE;
	}

	CanVolantData.CanVolantModuleData_2.INT_RollingCounter_Ctrl_Fbk	= pCanFrame->Data.RollingCounter_Ctrl & 0xFF;
}

/* ---------------------------------------------------------------------------------- */

void CanProtocol_MCU_INT2_TX_StateMsg_Update(void *pframe)
{
	...
	pCanFrame->Data.PowerFbk_H					= (uint8)((CAN_DATA_PACK(GsMSS_PowerCalc_numb, uint16, 400, 10) >> 8) & 0x1F);
	pCanFrame->Data.PowerFbk_L					= (uint8)(CAN_DATA_PACK(GsMSS_PowerCalc_numb, uint16, 400, 10) & 0xFF);
	...
	/* ----------------------------------CRC------------------------------------- */
	setA825CRC(sg_frameInfoList_INT2[MCU_INT2_TX_StateMsg].frameId, sg_frameInfoList_INT2[MCU_INT2_TX_StateMsg].dataLength, pframe);
	/* -----------------------------RollingCounter------------------------------- */
	CanVolantData.CanVolantModuleData_2.INT_RollingCounter_S++;
}

void CanProtocol_MCU_INT2_RX_StateMsg_Update(void *pframe)
{
	CANFD_INT_STATE_OBJ *pCanFrame = (CANFD_INT_STATE_OBJ *)pframe;

	if(checkA825CRC(sg_frameInfoList_INT2[MCU_INT2_RX_StateMsg].frameId, sg_frameInfoList_INT2[MCU_INT2_RX_StateMsg].dataLength, pframe))
	{
...
//		MS_Data.MS_StateMsg.CommunicationConfirm_Fbk					= pCanFrame->Data.CommunicationConfirm_Fbk & 0x0F;
		...
		GsBSW_MotPwrRd_Kw			                                    = CAN_DATA_ANSYS((CAN_DATA_CALC(pCanFrame->Data.PowerFbk_H, pCanFrame->Data.PowerFbk_L) & 0x1FFF), float, 400, 10);
...
//		MS_Data.MS_StateMsg.CRC											= CAN_DATA_ANSYS(CAN_DATA_CALC(pCanFrame->Data.CRC_H, pCanFrame->Data.CRC_L), float, 0, 1);
	...
	{
		CanVolantData.CanVolantModuleData_2.INT_CRC_State_Fault				= TRUE;
	}
}

uint32 CanProtocol_Volant_INT2_Event_Trigger(void)
{
	return 0;
}

void CanProtocol_Volant_INT2_Event_Update(void)
{
